function [input,output] = callNRLsise00(positionGCRF, dateAndTime)
...
hourValue   = hour(dateAndTime);
minuteValue = minute(dateAndTime);
secondValue = second(dateAndTime);

% Calculate SOD
sod = hourValue * 3600 + minuteValue * 60 + secondValue;

%%%Calculate lla
xyzout = eci2ecef(datevec(dateAndTime),positionGCRF);
llaSPG4 = ecef2lla(xyzout', 'WGS84');
    
%%%update input structure
input.doy  = day(dateAndTime, 'dayofyear');
input.year = dateAndTime.Year; % without effect
input.sec = sod;
input.alt=llaSPG4(3)*1E-3;
input.g_lat=llaSPG4(1);
input.g_long=llaSPG4(2);
input.lst=16;
input.f107A=150;
input.f107=150;
input.ap=4;
